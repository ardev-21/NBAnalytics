...
def procesar_team_details(file_path):
    ...
    team_details["arenacapacity"] = team_details["arenacapacity"].fillna(team_details["arenacapacity"].median())
...
def borrar_data():
    ...
    if os.path.exists(carpeta):
        ...
        print("Contenido de './data' eliminado correctamente.")
    else:
        print("La carpeta './data' no existe.")
        
if __name__ == '__main__':
    # Usar la API normalmente
    api.dataset_download_files('wyattowalsh/basketball', path='data/', unzip=True)
    # Directorio 
    DIRECTORY= './data/csv' 
    files = ['player', 'team', 'game', 'common_player_info', 
         'draft_combine_stats', 'draft_history', 'team_details',
         'team_history', 'game_info', 'game_summary', 'other_stats',
         'officials', 'inactive_players', 'play_by_play', 'line_score',
         ]
